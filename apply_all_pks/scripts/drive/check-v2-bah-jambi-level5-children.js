...
import { BAH_JAMBI_SHEETS_URL } from '../funcloc/config.js';
...
async function pickDriveFilesInteractive(driveFiles) {
  const mode = await select({
    message: 'Select Drive files to process:',
    choices: [
...
const parseArgs = () => {
  ...
  let driveSamples = null;
  let mode = 'sheet'; // sheet | drive-folder | local-xlsx
  let inputXlsx = null;
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (a === '--url' && args[i + 1]) {
      sheetUrl = args[i + 1];
      i += 1;
    } else if (a === '--drive-folder' && args[i + 1]) {
      driveFolderUrl = args[i + 1];
      mode = 'drive-folder';
      i += 1;
    } else if (a === '--drive-samples' && args[i + 1]) {
      const n = parseInt(args[i + 1], 10);
      if (!Number.isNaN(n) && n > 0) driveSamples = n;
      i += 1;
  ...
  return {
    ...
    driveSamples,
    inputXlsx: inputXlsx
      ? path.isAbsolute(inputXlsx)
        ? inputXlsx
        : path.join(process.cwd(), inputXlsx)
      : null,
...
const downloadSheetToTempXlsx = async (sheetUrl) => {
  const fileId = extractFileIdFromAnyUrl(sheetUrl);
  if (!fileId) {
    throw new Error(`Invalid Google Sheet URL/file ID: ${sheetUrl}`);
  }

  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bah-jambi-level5-'));
  const localPath = path.join(tempDir, 'bah-jambi.xlsx');
...
const processWorkbookFromXlsxPath = async ({ xlsxPath, sourceUrl, outDir, fixMapPath, outputPrefix }) => {
  ...
  return { outputXlsx, outputHtml, outputJson };
};

/**
 * Resolve a local path, `file:` URL, or Google Sheets/Drive file URL to a local .xlsx path.
 * @returns {{ localPath: string, cleanupDownloadDir: (() => void) | null }}
 */
const resolveWorkbookSourceToLocalPath = async (source) => {
  const s = String(source ?? '').trim();
  if (!s) throw new Error('Workbook source path or URL is empty');
  if (/^file:/i.test(s)) {
    const p = fileURLToPath(s);
    if (!fs.existsSync(p)) throw new Error(`file URL not found on disk: ${p}`);
    return { localPath: p, cleanupDownloadDir: null };
  }
  const fileId = extractFileIdFromAnyUrl(s);
  if (fileId) {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'bah-jambi-level5-src-'));
    const localPath = path.join(tmp, 'workbook.xlsx');
    await downloadFile(fileId, localPath);
    return {
      localPath,
      cleanupDownloadDir: () => {
  ...
  return { localPath: resolved, cleanupDownloadDir: null };
};

/**
 * Run Bah Jambi level-5 preprocessing; writes fixed .xlsx plus checker reports under `tempOutDir`.
 * Intended for chaining into apply-v2 (temp folder), or for programmatic use.
 *
 * @param {{ source: string, tempOutDir: string, fixMapPath?: string, outputPrefix?: string }} opts
 * @returns {Promise<{ outputXlsx: string, outputHtml: string, outputJson: string, cleanupDownloadDir: (() => void) | null }>}
 */
const preprocessBahJambiLevel5ForDraftApply = async ({
  source,
  tempOutDir,
  fixMapPath = DEFAULT_FIX_MAPPING_PATH,
  outputPrefix,
}) => {
  const { localPath: xlsxPath, cleanupDownloadDir } = await resolveWorkbookSourceToLocalPath(source);
  try {
    const fp = path.isAbsolute(fixMapPath) ? fixMapPath : path.resolve(process.cwd(), fixMapPath);
    fs.mkdirSync(tempOutDir, { recursive: true });
    const prefix =
      outputPrefix != null && String(outputPrefix).trim() !== ''
        ? String(outputPrefix).trim()
        : path.basename(xlsxPath, path.extname(xlsxPath));
    const result = await processWorkbookFromXlsxPath({
      xlsxPath,
    ...
    return { ...result, cleanupDownloadDir };
  } catch (err) {
    if (cleanupDownloadDir) cleanupDownloadDir();
    throw err;
  }
};

const runInteractive = async () => {
  console.log('\n=== Bah Jambi Level-5 Checker (Interactive) ===');
  console.log('↑/↓ move • Enter select\n');

  const mode = await select({
    message: 'Input source?',
    choices: [
      { name: 'Master data (single Google Sheet / Drive file URL)', value: 'sheet' },
      { name: 'Local .xlsx file (single)', value: 'local-xlsx' },
      { name: 'Google Drive folder (many files)', value: 'drive-folder' },
    ],
    default: 'sheet',
    ...SELECT_OPTS,
  });

  const fixMapPathRaw = await input({
    message: 'Fix mapping JSON path (--fix-map)',
    default: DEFAULT_FIX_MAPPING_PATH,
  });
  ...
  const fixMapPath = path.isAbsolute(fixMapPathRaw)
    ? fixMapPathRaw
    : path.join(process.cwd(), fixMapPathRaw);
  const outDir = path.isAbsolute(outDirRaw) ? outDirRaw : path.join(process.cwd(), outDirRaw);

  let sheetUrl = DEFAULT_SHEET_URL;
  let inputXlsx = null;
  let driveFolderUrl = DEFAULT_DRIVE_FOLDER_URL;
  let drivePick = 'sample';

  if (mode === 'sheet') {
    sheetUrl = await input({
      message: 'Master data sheet/file URL (--url)',
      default: DEFAULT_SHEET_URL,
    });
  } else if (mode === 'local-xlsx') {
    const p = await input({
      message: 'Path to local .xlsx (--input)',
      validate: (v) => {
        const resolved = path.isAbsolute(String(v ?? ''))
          ? String(v ?? '')
          : path.join(process.cwd(), String(v ?? ''));
        return fs.existsSync(resolved) ? true : 'File not found';
      },
    });
    inputXlsx = path.isAbsolute(p) ? p : path.join(process.cwd(), p);
  } else {
    driveFolderUrl = await input({
      message: 'Google Drive folder URL (--drive-folder)',
      default: DEFAULT_DRIVE_FOLDER_URL,
    });
    drivePick = await select({
      message: 'Drive files selection',
      choices: [
        { name: 'All files', value: 'all' },
        { name: 'Pick sample (folders / first N / pick files)', value: 'sample' },
      ],
      default: 'sample',
      ...SELECT_OPTS,
    });
  ...
  const runNow = await confirm({
    message: 'Run now with these settings?',
    default: true,
  });
  if (!runNow) process.exit(0);

  return { mode, sheetUrl, inputXlsx, driveFolderUrl, drivePick, outDir, fixMapPath };
};

const main = async () => {
  const rawArgs = process.argv.slice(2);
  const useInteractive = rawArgs.length === 0 && Boolean(process.stdout.isTTY);
  const args = useInteractive ? await runInteractive() : parseArgs();
  if (!useInteractive && args.help) {
    console.log(`
Bah Jambi Level-5 Checker

Interactive:
  node scripts/drive/check-bah-jambi-level5-children.js

Flags:
  --url <sheetOrDriveFileUrl>        Master data source (default: BAH_JAMBI_SHEETS_URL)
  --input <local.xlsx>               Process one local workbook
  --drive-folder <folderUrl>         Process all (or sample) Excel files from Drive folder
  --drive                            Alias for --drive-folder (uses default folder)
  --drive-samples <N>                Drive batch: first N files (non-interactive)
...
`);
  ...
  const { mode, sheetUrl, outDir, fixMapPath, driveFolderUrl, driveSamples, inputXlsx } = args;
  ...
  if (mode === 'local-xlsx') {
    if (!inputXlsx || !fs.existsSync(inputXlsx)) {
      throw new Error(`Input xlsx not found: ${inputXlsx || '(missing --input)'}`);
    }
    await processWorkbookFromXlsxPath({
      xlsxPath: inputXlsx,
      sourceUrl: inputXlsx,
      outDir,
      fixMapPath,
      outputPrefix: path.basename(inputXlsx, path.extname(inputXlsx)),
    });
    return;
  }
  ...
  if (mode === 'drive-folder') {
    ...
    if (SOURCE_DRIVE_OWNER_EMAIL) console.log(`Drive owner filter: ${SOURCE_DRIVE_OWNER_EMAIL}`);

    const allFiles = await listFilesRecursively(folderId, '', null);
    let { driveFiles } = filterDriveSourceExcelFiles(allFiles, {
      allowedOwnerEmails: SOURCE_DRIVE_OWNER_EMAIL ? [SOURCE_DRIVE_OWNER_EMAIL] : [],
      logInfo: (s) => console.log(s),
      formatOwnerExcludedCount: (n) =>
        SOURCE_DRIVE_OWNER_EMAIL ? `Filtered out ${n} file(s) not owned by ${SOURCE_DRIVE_OWNER_EMAIL}` : '',
    });
    if (!driveFiles.length) throw new Error('No Excel files found in Drive folder.');

    if (useInteractive && args.drivePick === 'sample') {
      const picked = await pickDriveFilesInteractive(driveFiles);
      driveFiles = picked.files;
      console.log(`Drive selection: ${picked.summary}`);
    } else if (!useInteractive && driveSamples != null) {
      driveFiles = driveFiles.slice(0, driveSamples);
      console.log(`Drive samples: first ${driveFiles.length}`);
    }
  ...
  // mode === 'sheet'
  console.log(`Source: ${sheetUrl}`);
  let tempDir = null;
  try {
    const downloaded = await downloadSheetToTempXlsx(sheetUrl);
    tempDir = downloaded.tempDir;
    await processWorkbookFromXlsxPath({
      xlsxPath: downloaded.localPath,
      sourceUrl: sheetUrl,
      outDir,
      fixMapPath,
      outputPrefix: 'master',
...
export {
  processWorkbookFromXlsxPath,
  preprocessBahJambiLevel5ForDraftApply,
  resolveWorkbookSourceToLocalPath,
  DEFAULT_FIX_MAPPING_PATH,
};
