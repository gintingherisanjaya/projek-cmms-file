/**
 * lsmw_measuring_point.cjs
 * Konversi Protect Y → template LSMW measuring point (hourmeter).
 *
 *   pnpm run lsmw-measuring-point
 *
 * Prompt: URL Protect Y, pilih subfolder REGIONAL* (checkbox), mode validasi panjang karakter.
 * Output: Output/lsmw-measuring-point/{timestamp WIB}/REGIONAL N/{nama PKS}.xlsx + {REGIONAL N}.xlsx + all-pks.xlsx + validation.xlsx
 */
...
const { loadMeasuringPointTemplateLimits } = require("./utils/lsmw_measuring_point_limits.cjs");
const {
    OUTPUT_COLUMN_ORDER,
    buildMeasuringPointRows
} = require("./utils/lsmw_measuring_point_transform.cjs");
const { writeMeasuringPointExcel } = require("./utils/lsmw_measuring_point_excel.cjs");
...
const OUTPUT_DIR = path.join("Output", "lsmw-measuring-point");
const TEMPLATE_PATH = "./template_measuring_point.xlsx";
...
async function main() {
    ...
    try {
        ...
        const templateLimits = await loadMeasuringPointTemplateLimits(
        ...
        for (const { regional, file } of entries) {
            ...
            if (idxFuncLoc !== undefined) {
                for (let i = 0; i < dataRows.length; i++) {
                    ...
                    workCenterByRowIndex.set(i, workCenter);
                }
            }
            ...
            const built = buildMeasuringPointRows(
                sheet.rawRows,
                file.name,
            ...
            if (built.violations.length > 0) {
                if (charMode === "strict") {
                    ...
                    stopWithError(formatViolation(first));
                }
            ...
            await writeMeasuringPointExcel(
                outPath,
                path.resolve(TEMPLATE_PATH),
                built.rows,
                columnOrder
        ...
        console.log(`  Total baris measuring point: ${totalRows}`);
        console.log(
